...
public class DbConnectionTest {
    ...
    public void testDbConnection() {
        ...
        try {
            conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/addressbook8?user=root&password=");
            Statement st = conn.createStatement();
            ResultSet resultSet = st.executeQuery("select group_id,group_name,group_header,group_footer from group_list");
            Groups groups = new Groups();
            while (resultSet.next()) {
                groups.add(new GroupData().withId(resultSet.getInt("group_id")).withName(resultSet.getNString("group_name"))
